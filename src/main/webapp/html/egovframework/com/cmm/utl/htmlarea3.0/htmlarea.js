 ...
 *   수정일         수정자           Function 명
 * ------------    ---------    ----------------------------
 * 2011.09.01	    이기하			_editor_url은 호출하는 jsp에서 받아오도록 설정
 *                                  (contextpath 설정을 위함)
 *
 *
 */
...
//웹에디터 초기화
//_editor_url = "/html/egovframework/com/cmm/utl/htmlarea3.0/";
_editor_lang = "kr";

var editor = null;
function initEditor() {

	editor = new HTMLArea(_editor_area);
	...
	this._popupDialog("insert_table.html", function(param) {
		...
		for (var field in param) {
			...
			switch (field) {
			    ...
			    case "f_summary" : table.summary = value; break;	// summary 추가
			}
		}
		//------------------
		// caption 추가
		...
		if (param.f_caption) {
			...
			caption.innerHTML = param.f_caption;
		...
		if (param.f_thead) {
			...
			for (var j = 0; j < param["f_cols"]; ++j) {
				...
				(HTMLArea.is_gecko) && th.appendChild(doc.createElement("br"));
			}
		...
		//-----------------------------------------------
		// 웹접근성 관련 처리
		// (table의 경우 p, i, b, u 등의 tag 안에 올 수 없음)
		//-----------------------------------------------
		for (i = 0; i < ancestors.length; ++i) {
			if (isCheckTarget(ancestors[i])) {
				makeMagic(ancestors[i], table, i, doc);
			}
		}
		////---------------------------------------------
		return true;
...
function isCheckTarget(el) {
	var checkTarget = " p i b u strike ";
	return el && el.nodeType == 1 && (checkTarget.indexOf(" " + el.tagName.toLowerCase() + " ") != -1);
};

function makeMagic(target, table, index, doc) {
	var parent = target.parentNode;

	var first, second, attrs, i, n, front, tmp;

	first = doc.createElement(target.nodeName);		// 앞부분
	second = doc.createElement(target.nodeName);	// 뒤부분

	// 속성 복사 처리
	attrs = target.attributes;
	for (i = 0; i < attrs.length; i++) {
		//alert(attrs[i].name + " : " + attrs[i].value);
		first.setAttribute(attrs[i].name, attrs[i].value);
		second.setAttribute(attrs[i].name, attrs[i].value);
	}

	// Text Node 처리
	front = true;
	for (n = target.firstChild; n; n = n.nextSibling) {
		if (n == table || n.nodeName.toLowerCase() == 'table') {
			front = false;
			continue;
		}

		if (n.nodeType == 3) {	// TEXT_NODE
			//alert(n.nodeName + " : " + n.nodeValue);
			tmp = doc.createTextNode(n.nodeValue);
		} else if (n.nodeType == 1) {	// ELEMENT_NODE
			tmp = doc.createElement(n.nodeName);

			//// 속성 복사 처리
			//attrs = n.attributes;
			//for (i = 0; i < attrs.length; i++) {
			//	//alert(attrs[i].name + " : " + attrs[i].value);
			//	tmp.setAttribute(attrs[i].name, attrs[i].value);
			//}

			copyChildren(n, tmp, doc);
		}

		if (front) {
			first.appendChild(tmp);
		} else {
			second.appendChild(tmp);
		}
	}
	// 차상위 node에 대하여 위치를 잡을 수 없음.. (But 웹표준은 준수됨..)
	// ex: <p>aaa<u>bb{table위치}cc</u>ddd</p>  ->  <p>aaaddd</p><u>bb</u><table/><u>cc</u><p></p>
	/*
	parent.appendChild(first);
	parent.appendChild(table);
	parent.appendChild(second);
	*/
	// table이 의도된 위치로 지정됨.. (But 웹표준은 준수되지 않음..)
	// ex: <p>aaa<u>bb{table위치}cc</u>ddd</p>  ->  <p>aaa<u>bb</u><table/><u>cc</u>ddd</p>
	parent.insertBefore(first, target);
	parent.insertBefore(table, target);
	parent.insertBefore(second, target);
...
function copyChildren(source, target, doc) {
	var n, foster;
	var parent = target.parentNode;
	...
	for (n = source.firstChild; n; n = n.nextSibling) {
		//alert(source.nodeName + " has a " + n.nodeName + " : " + n.nodeValue);
		if (n.nodeType == 3) {	// TEXT_NODE
			foster = doc.createTextNode(n.nodeValue);
		} else if (n.nodeType == 1) {	// ELEMENT_NODE
			foster = doc.createElement(n.nodeName);
		}

		target.appendChild(foster);

		if (n.nodeType == 1) {	// ELEMENT_NODE만 하위 적용...
			copyChildren(n, foster, doc);
			//alert('before remove : ' + n.nodeName);
...
HTMLArea.prototype.execCommand = function(cmdID, UI, param) {
	...
	switch (cmdID) {
		...
		if (HTMLArea.is_ie) {
			...
			{
				window.open(this.popupURL("fullscreen.jsp"), "ha_fullscreen",
